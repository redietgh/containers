...
These two classes are the building blocks
for the BST, AVLTree, and Heap data structures.
It is crucial to get these implemented correctly
in order to be able to implement the other data structures.
'''
...
class BinaryTree():
    ...
    def print_tree(self, traversal_type):
        ...
        for you as a reference on how to
        write recursive functions on recursive data structures.
        '''
        if traversal_type == 'preorder':
            ...
            raise ValueError(f"Traversal type {traversal_type} \
                    is not supported.")
    ...
    def to_list(self, traversal_type):
        ...
        we can always print the returned results
        if we need to visualize them.
        But by returning the results we can also
        do more computations on the results if needed.
